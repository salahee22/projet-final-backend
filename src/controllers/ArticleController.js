...
class ArticleController {
 ...
 async createArticle(req, res, next) {
  try {
    const {
  title, summary, content, category, age, image, images, video, read_time, tag,
  intro, chapters, conclusion, sidebar,
} = req.body;
...
const article = await Article.create({
  ...
  intro, chapters, conclusion, sidebar,
    ...
    res.status(201).json({ success: true, message: "Article created", data: populated });
  } catch (error) {
    next(this.normalizeError(error));
  }
}
  ...
  try {
    ...
    const isOwner = article.author_id.toString() === req.user.id.toString();
    const isAdmin = req.user.role === "admin";
    if (!isOwner && !isAdmin) return next(createError(403, "Forbidden: you can only edit your own articles"));
...
const updated = await Article.findByIdAndUpdate(
  req.params.id,
  { title, summary, content, category, age, image, images, video, read_time, tag, intro, chapters, conclusion, sidebar },
